...
admins can use `/togglelog download` to keep normal INFO logging while enabling editable `/play` download log messages in Discord. `/togglelog debug` enables both DEBUG logging and the download log, while `/togglelog admin` and `/togglelog all` enable the larger user-space operation trail. `/play show_download_log:true` shows the same sanitized progress log for one request without changing the global toggle. the message is posted before voice connection work, then edited through voice join, metadata extraction, cache lookup, download progress, and ffmpeg startup. when yt-dlp reports byte totals it includes an ASCII progress bar, downloaded amount, speed, cache state, format id, and final ffmpeg playback path without exposing local absolute file paths. reacting with the cleanup emoji collapses the log back to a normal summary.
